...
import Video from "next-video"; // Импортируем плеер из next-video
...
export const PhotoItem: React.FC<PhotoItemProps> = ({ photo, index, onDelete, photos, globalType }) => {
  const iconColor = fileColors[photo.type.split('/')[1]] || fileColors.unknown;

  return (
    <>
      {globalType === "imgs" ? (
        photo.type.startsWith("video/") ? (
          // Используем плеер из next-video для видеофайлов
          <div
            key={index}
            className={`relative ${
          ...
          // Отображаем изображение, если файл — фото
          <div
            key={index}
            className={`relative ${
              photos.length % 2 !== 0 && index === photos.length - 1
        ...
        <div className="flex items-center justify-between p-2">
          <div className={`flex items-center ${iconColor} w-[48px] h-[48px] justify-center rounded-[6px]`}>
            <span className="text-white">{truncateUrl(photo.type.split('/')[1], 4)}</span>
          </div>

...
const Message: FC<MessageProps> = ({ message, channel, profile, setIsReplying }) => {
  ...
  return (
    <div key={message.id} className={`flex items-center mb-4 ${isOwn ? "justify-end" : "justify-start"}`}>
        ...
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
          <PopoverTrigger>
            <div
              ...
              className={`relative min-h-[32px] flex flex-col justify-center px-2 max-w-[25rem] ${
                isOwn ? "bg-[#766ac8] text-black" : "bg-[#212121] text-white"
              }`}
            >
              {message.replyToMessage && (
                <div
                  className={`mt-2 mb-1 min-h-[40px] rounded-[10px] relative flex items-center ${
                    isOwn ? "bg-[#5d4fa7]" : "bg-[#353535]"
                  }`}
                >
              ...
              <div
                className={`overflow-auto ${
                  message.files?.type === "imgs" ? "grid gap-2 h-full grid-cols-2" : "flex flex-col"
                }`}
              >
                {message.files?.type === "imgs"
                  ? message.files?.fileUrls?.map((photo: any, index: number) => (
                      <div
                        ...
                        className={`relative mt-2 ${
                          message.files.fileUrls.length % 2 !== 0 && index === message.files.fileUrls.length - 1
                            ? "col-span-2"
                            : ""
                        }`}
                      >
                  ...
                  : message.files?.fileUrls.map((file: any) => {
                      const iconColor =
                        fileColors[file.mimetype?.split("/")[file.mimetype.split("/").length - 1]] || fileColors.unknown;
                      return (
      ...
      {isOwn && <img src={profile.imageUrl} alt="Current user's avatar" className="w-10 h-10 rounded-full ml-4" />}
    </div>
  );
};

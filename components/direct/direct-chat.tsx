...
import {ChannelChatProps, ChannelType} from "@/types/Channel";
...
const DirectChat: FC<DirectProps> = ({ conversation, profile }) => {
    const queryKey = `direct:${conversation.id}`
    const addKey = `direct:${conversation.id}:messages`
    const updateKey = `direct:${conversation.id}:messages:update`
